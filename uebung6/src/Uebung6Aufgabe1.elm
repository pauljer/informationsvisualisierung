module Uebung6Aufgabe1 exposing (main)
...
import TypedSvg.Attributes as TA exposing (transform, viewBox, stroke, fill)
    ...
    let
        ...
        axesNodes =
            List.indexedMap
                (\idx desc ->
                    let
                        xPos = Scale.convert xScale (toFloat (idx + 1))
                        scaleY = getYScale idx
                    in
...
filterCars list =
    list
        |> List.filter (\c -> 
               c.carType == SUV &&
               c.cityMPG /= Nothing &&
               c.retailPrice /= Nothing &&
               c.dealerCost /= Nothing &&
               c.carLen /= Nothing
           )
...
carsToMultiDimData list =
    let
        filtered = filterCars list
        
        points =
            filtered
                |> List.filterMap (\c ->
                    Maybe.map4 (\city ret deal len ->
                        { pointName = c.vehicleName
                        , value = [ toFloat city, toFloat ret, toFloat deal, toFloat len ]
                        }
                    ) c.cityMPG c.retailPrice c.dealerCost c.carLen
                )
    in
...
main =
    let
        filtered = filterCars cars
        mdData = carsToMultiDimData cars
    in
    Html.div
