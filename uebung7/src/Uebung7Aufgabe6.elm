module Uebung7Aufgabe6 exposing (..)
...
init _ =
    ...
    in
    ( { files = initialFiles, currentScale = Viridis, currentIndex = "DJ" }
    , Cmd.batch commands
    )
...
csvString_to_data csvRaw =
    let
        ...
        -- Erkennt automatisch, ob Komma oder Semikolon als Trennzeichen genutzt wird
        separator =
...
view model =
    div [ TypedSvg.Attributes.style "padding: 20px; font-family: sans-serif;" ]
        [ h3 [] [ text "Aufgabe 7.6: Farbskalen und Index auswählen" ]
        ...
        , case Dict.get model.currentIndex model.files of
            Just (Success fullData) ->
                renderPattern model.currentScale fullData
