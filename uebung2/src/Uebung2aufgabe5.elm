module Uebung2aufgabe5 exposing (main)
...
retailPriceValue : Car -> Float
retailPriceValue car =
    car.retailPrice
        |> Maybe.withDefault 0
        |> toFloat


-- Umrechnung MPG (US) -> Liter pro 100 km
mpgToLper100km : Float -> Float
mpgToLper100km mpg =
    if mpg <= 0 then
        0
...
cityLper100km : Car -> Float
cityLper100km car =
    mpgToLper100km (cityMpg car)


averageOf : (Car -> Float) -> List Car -> Float
averageOf metric carList =
    let
        values =
            List.map metric carList
    in
    case values of
        [] ->
            0
...
averageCityMPG =
    averageOf cityMpg carsInClass


averageCityLper100km : Float
averageCityLper100km =
    averageOf cityLper100km carsInClass


-- Teilmengen auf Basis MPG (Original): unter Durchschnitt = schlechter,
-- über/gleich Durchschnitt = besser
belowAverageMpg : List Car
belowAverageMpg =
    List.filter (\car -> cityMpg car < averageCityMPG) carsInClass


aboveAverageMpg : List Car
aboveAverageMpg =
    List.filter (\car -> cityMpg car >= averageCityMPG) carsInClass


-- Teilmengen auf Basis L/100km (Neu): hoher Verbrauch = schlechter,
-- niedriger Verbrauch = besser. (2) und (3) werden neu berechnet.
belowAverageLper100km : List Car
belowAverageLper100km =
    List.filter (\car -> cityLper100km car > averageCityLper100km) carsInClass


aboveAverageLper100km : List Car
aboveAverageLper100km =
    List.filter (\car -> cityLper100km car <= averageCityLper100km) carsInClass
...
groupColors : PointGroup -> { fill : String, stroke : String, strokeWidth : String, opacity : String }
groupColors pointGroup =
    case pointGroup of
        OutsideClass ->
            { fill = "#cbd2d9", stroke = "#9aa5b1", strokeWidth = "1", opacity = "0.5" }

        BelowClassAverage ->
            { fill = "#4c6ef5", stroke = "#364fc7", strokeWidth = "1", opacity = "0.75" }

        AboveClassAverage ->
            { fill = "#ff5a36", stroke = "#8f1d14", strokeWidth = "2", opacity = "1" }


renderPoint : PointGroup -> Float -> Float -> Svg msg
renderPoint pointGroup xPos yPos =
    let
        s =
            groupColors pointGroup
    in
    circle
        ...
        , cy yPos
        , r 5
        , TypedSvg.Core.attribute "fill" s.fill
        , TypedSvg.Core.attribute "stroke" s.stroke
        , TypedSvg.Core.attribute "stroke-width" s.strokeWidth
        , TypedSvg.Core.attribute "opacity" s.opacity
        ]
        []


renderPoints : (Car -> Float) -> PointGroup -> List Car -> Extent -> Extent -> List (Svg msg)
renderPoints xMetric pointGroup carsToRender xExtent yExtent =
    List.map
        (\car ->
            renderPoint
                pointGroup
                (scaleX xExtent (xMetric car))
                (scaleY yExtent (retailPriceValue car))
        )
        carsToRender
...
formatTick value =
    let
        rounded =
            (value * 10 |> round |> toFloat) / 10
    in
    String.fromFloat rounded


legendBox : Float -> PointGroup -> String -> Svg msg
legendBox yPos pointGroup labelText =
    g []
        [ renderPoint pointGroup (plotWidth - 200) yPos
        , text_
            [ x (plotWidth - 186)
            , y (yPos + 4)
            , class [ "tick-label" ]
...
legend =
    g []
        [ legendBox 36 OutsideClass "nicht in Klasse"
        , legendBox 58 BelowClassAverage "schlechter als Durchschnitt"
        , legendBox 80 AboveClassAverage "besser als Durchschnitt"
        ]
...
scatterplot : PlotSpec -> Svg msg
scatterplot spec =
    let
        allCars =
            notInClass ++ carsInClass

        xExtent =
            extentWithPadding (List.map spec.xMetric allCars)

        yExtent =
            extentWithPadding (List.map retailPriceValue allCars)

        averageLineX =
            scaleX xExtent spec.averageValue
    in
    svg [ viewBox 0 0 plotWidth plotHeight, width plotWidth, height plotHeight ]
        ([ style [] [ TypedSvg.Core.text axisStyles ]
         ...
         , text_
            ...
            [ TypedSvg.Core.text spec.averageLabel ]
         , text_
            [ x (plotWidth / 2)
            , y (plotHeight - 18)
            , textAnchor AnchorMiddle
            ...
            [ TypedSvg.Core.text spec.xLabel ]
         , text_
            [ x 14
            , y 30
            , class [ "axis-label" ]
            ...
            ++ xTicks xExtent
            ++ yTicks yExtent
            ++ renderPoints spec.xMetric OutsideClass notInClass xExtent yExtent
            ++ renderPoints spec.xMetric BelowClassAverage spec.below xExtent yExtent
            ++ renderPoints spec.xMetric AboveClassAverage spec.above xExtent yExtent
            ++ [ legend ]
        )
...
mpgSpec =
    { xMetric = cityMpg
    , xLabel = "cityMPG"
    , averageValue = averageCityMPG
    , averageLabel = "avg = " ++ formatFloat2 averageCityMPG ++ " MPG"
    , below = belowAverageMpg
    , above = aboveAverageMpg
    }


lper100kmSpec : PlotSpec
lper100kmSpec =
    { xMetric = cityLper100km
    , xLabel = "cityVerbrauch [L / 100 km]"
    , averageValue = averageCityLper100km
    , averageLabel = "avg = " ++ formatFloat2 averageCityLper100km ++ " L/100km"
    , below = belowAverageLper100km
    , above = aboveAverageLper100km
    }


plotColumn : String -> String -> PlotSpec -> Html msg
plotColumn titleText description spec =
    Html.div
        [ Html.Attributes.style "flex" "1"
        , Html.Attributes.style "min-width" "520px"
        ]
