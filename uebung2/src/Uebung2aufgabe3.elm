module Uebung2aufgabe3 exposing (main)
...
formatFloat decimals value =
    let
        ...
        roundedValue =
            (value * factor |> round |> toFloat) / factor
    in
...
pointClass : PointGroup -> String
pointClass pointGroup =
    case pointGroup of
        OutsideClass ->
            "outside-class"
...
renderPoint variant pointGroup xPos yPos =
    case ( variant, pointGroup ) of
        ( FocusByShape, AboveClassAverage ) ->
            g
                [ transform [ Translate xPos yPos, Rotate 45 0 0 ]
                , class [ pointClass pointGroup ]
                ]
                [ rect [ x -8, y -8, width 16, height 16 ] [] ]

        ( FocusBySize, AboveClassAverage ) ->
            circle
                [ cx xPos
                , cy yPos
                , r 9
        ...
        ([ style [] [ TypedSvg.Core.text (styleSheet variant) ]
         ...
         , text_
            ...
            , class [ "boundary-label" ]
                ...
                [ TypedSvg.Core.text ("avg cityMPG = " ++ String.fromFloat averageCityMPG) ]
         , text_
            [ x (plotWidth / 2)
            , y (plotHeight - 18)
            , textAnchor AnchorMiddle
...
main =
    Html.div
        ...
        , Html.p [] [ Html.text ("Durchschnitt cityMPG: " ++ String.fromFloat averageCityMPG) ]
        , Html.p []
            [ Html.text "Jede der folgenden drei Darstellungen zeigt dieselben drei Teilmengen in einem gemeinsamen Scatterplot mit den Achsen cityMPG und retailPrice. Die rote gestrichelte Linie markiert den durchschnittlichen Verbrauch der gewählten Klasse." ]
        , plotSection
            "Version 1: Hervorhebung über Farbe"
            "Die Teilmenge oberhalb des Durchschnitts fällt durch einen starken Farbkontrast sofort ins Auge."
            FocusByColor
        , plotSection
            "Version 2: Hervorhebung über Größe"
            "Hier wird die auffällige Teilmenge mit deutlich größeren Punkten gezeigt, während die anderen Gruppen kleiner bleiben."
            FocusBySize
        , plotSection
            "Version 3: Hervorhebung über Form"
            "In dieser Variante wird die auffällige Teilmenge durch gedrehte Quadrate statt Kreise kodiert."
            FocusByShape
        ]