module Uebung2aufgabe2 exposing (main)
...
-- 2. Filter: nur vollständige Autos
isComplete : Car -> Bool
isComplete car =
...
-- 3. Autos in gewählter Klasse
carsInClass : List Car
carsInClass =
    List.filter (\c -> c.carType == selectedClass) filteredCars
...
averageCityMPG =
    let
        mpgValues =
            List.filterMap (\c -> Maybe.map toFloat c.cityMPG) carsInClass
    in
    case mpgValues of
        [] ->
            0
...
main =
    Html.div []
        [ Html.p []
            [ Html.text ("Gewählte Klasse: " ++ carTypeToString selectedClass) ]
        , Html.p []
            [ Html.text ("Anzahl nicht in Klasse: " ++ String.fromInt (List.length notInClass)) ]
        , Html.p []
            [ Html.text ("Anzahl unter Durchschnitt: " ++ String.fromInt (List.length belowAverage)) ]
        , Html.p []
            [ Html.text ("Anzahl über Durchschnitt: " ++ String.fromInt (List.length aboveAverage)) ]
        , Html.p []
            [ Html.text ("Durchschnitt cityMPG: " ++ String.fromFloat averageCityMPG) ]
        ]