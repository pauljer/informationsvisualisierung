module Uebung2aufgabe4 exposing (main)
...
averageCityMPG : Float
averageCityMPG =
    let
        mpgValues =
            List.map cityMpg carsInClass
    in
    case mpgValues of
        [] ->
            0
        ...
        _ ->
            List.sum mpgValues / toFloat (List.length mpgValues)
...
belowAverage =
    List.filter (\car -> cityMpg car < averageCityMPG) carsInClass
    ...
    let
        ...
        paddingValue =
            if rawRange == 0 then
                ...
                rawRange * 0.08
    in
    { min = max 0 (minValue - paddingValue)
...
scale value domainMin domainMax rangeMin rangeMax =
    if domainMax == domainMin then
        ...
        in
        rangeMin + ratio * (rangeMax - rangeMin)
...
pointStyle : PlotVariant -> PointGroup -> PointStyle
pointStyle variant pointGroup =
    case ( variant, pointGroup ) of
        ( FocusBySaturation, OutsideClass ) ->
            { fill = "#e4e7eb", stroke = "#cbd2d9", strokeWidth = "1", opacity = "0.6" }

        ( FocusBySaturation, BelowClassAverage ) ->
            { fill = "#94a3b8", stroke = "#64748b", strokeWidth = "1", opacity = "0.85" }

        ( FocusBySaturation, AboveClassAverage ) ->
            { fill = "#b91c1c", stroke = "#7f1d1d", strokeWidth = "1.5", opacity = "1" }

        ( FocusByOutline, OutsideClass ) ->
            { fill = "#cbd2d9", stroke = "#9aa5b1", strokeWidth = "1", opacity = "0.6" }

        ( FocusByOutline, BelowClassAverage ) ->
            { fill = "#cbd2d9", stroke = "#1f2933", strokeWidth = "1.5", opacity = "0.9" }

        ( FocusByOutline, AboveClassAverage ) ->
            { fill = "#ffffff", stroke = "#0b3d91", strokeWidth = "3", opacity = "1" }


renderPoint : PlotVariant -> PointGroup -> Float -> Float -> Svg msg
renderPoint variant pointGroup xPos yPos =
    let
        s =
            pointStyle variant pointGroup
    in
    circle
        ...
        , cy yPos
        , r 5
        , attribute "fill" s.fill
        , attribute "stroke" s.stroke
        , attribute "stroke-width" s.strokeWidth
        , attribute "opacity" s.opacity
        ]
        []


renderPoints : PlotVariant -> PointGroup -> List Car -> Extent -> Extent -> List (Svg msg)
renderPoints variant pointGroup carsToRender xExtent yExtent =
    List.map
        (\car ->
            renderPoint
                variant
                pointGroup
                (scaleX xExtent (dealerCostValue car))
                (scaleY yExtent (carLenValue car))
        )
        carsToRender
        ...
        |> List.concatMap
            (\tickIndex ->
                ...
                , text_
                    ...
                    [ TypedSvg.Core.text (String.fromInt (round tickValue)) ]
                ]
            )
...
legendItem : PlotVariant -> PointGroup -> Float -> Float -> String -> Svg msg
legendItem variant pointGroup xPos yPos labelText =
    g []
        [ renderPoint variant pointGroup xPos yPos
        , text_
            [ x (xPos + 14)
...
legend : PlotVariant -> Svg msg
legend variant =
    g []
        [ legendItem variant OutsideClass (plotWidth - 240) 36 "nicht in Klasse"
        , legendItem variant BelowClassAverage (plotWidth - 240) 58 "unter Durchschnitt"
        , legendItem variant AboveClassAverage (plotWidth - 240) 80 "über Durchschnitt"
        ]


scatterplot : PlotVariant -> Svg msg
scatterplot variant =
    let
        allCars =
            notInClass ++ belowAverage ++ aboveAverage

        xExtent =
            extentWithPadding (List.map dealerCostValue allCars)

        yExtent =
            extentWithPadding (List.map carLenValue allCars)
    in
    svg [ viewBox 0 0 plotWidth plotHeight, width plotWidth, height plotHeight ]
        ([ style [] [ TypedSvg.Core.text styleSheet ]
         , line
            [ x1 padding
            , y1 (plotHeight - padding)
         ...
         , text_
            ...
            ++ xTicks xExtent
            ++ yTicks yExtent
            ++ renderPoints variant OutsideClass notInClass xExtent yExtent
            ++ renderPoints variant BelowClassAverage belowAverage xExtent yExtent
            ++ renderPoints variant AboveClassAverage aboveAverage xExtent yExtent
            ++ [ legend variant ]
        )


plotSection : String -> String -> PlotVariant -> Html msg
plotSection titleText description variant =
    Html.div [ Html.Attributes.style "margin-top" "32px" ]
        [ Html.h3 [] [ Html.text titleText ]
...
main =
    Html.div
        ...
        [ Html.h2 [] [ Html.text "Aufgabe 2.4" ]
        , Html.p [] [ Html.text ("Gewählte Klasse: " ++ carTypeToString selectedClass) ]
        , Html.p [] [ Html.text ("Anzahl nicht in Klasse: " ++ String.fromInt (List.length notInClass)) ]
        , Html.p [] [ Html.text ("Anzahl unter Durchschnitt: " ++ String.fromInt (List.length belowAverage)) ]
        , Html.p [] [ Html.text ("Anzahl über Durchschnitt: " ++ String.fromInt (List.length aboveAverage)) ]
        , Html.p [] [ Html.text ("Durchschnitt cityMPG (zur Gruppenbildung): " ++ String.fromFloat averageCityMPG) ]
        , Html.p []
            [ Html.text "Beide Darstellungen zeigen dieselben drei Teilmengen aus Aufgabe 2.3 in einem gemeinsamen Scatterplot mit den Achsen dealerCost und carLen. Da cityMPG hier nicht auf einer Achse liegt, kann die Entscheidungsgrenze nicht direkt eingezeichnet werden. Stattdessen wird die Teilmenge oberhalb des Durchschnitts durch andere visuelle Eigenschaften als in Aufgabe 2.3 hervorgehoben." ]
        , plotSection
            "Version 1: Hervorhebung über Sättigung und Helligkeit"
            "Die Punkte außerhalb der Klasse erscheinen sehr hell und entsättigt, die Klasse unter dem Durchschnitt mittelgrau und gedämpft, die Teilmenge oberhalb des Durchschnitts in einem kräftigen, gesättigten Rot. So entsteht ein klarer Kontrast in der Sättigung."
            FocusBySaturation
        , plotSection
            "Version 2: Hervorhebung über Umrandung und Füllung"
            "Hier haben alle Gruppen dieselbe Größe und Form, aber die Teilmenge oberhalb des Durchschnitts wird mit einer auffällig dicken, dunklen Umrandung und heller Füllung gezeigt, während die anderen Gruppen entweder gar keine starke Kontur oder nur eine dünne dunkle Linie haben."
            FocusByOutline
        ]
