																...
																return function (coalDerivedGas) {
																	return function (waste) {
																		return function (others) {
																			return {biomass: biomass, brownCoal: brownCoal, coalDerivedGas: coalDerivedGas, countryId: countryId, gas: gas, geothermal: geothermal, hardCoal: hardCoal, hydroPumped: hydroPumped, hydroReservoir: hydroReservoir, hydroRor: hydroRor, load: load, nuclear: nuclear, oil: oil, others: others, solar: solar, unixSeconds: unixSeconds, waste: waste, windOffshore: windOffshore, windOnshore: windOnshore};
																		};
																	};
																};
															};
		...
		switch (msg.$) {
			case 'TokenInput':
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'Connect':
				var manual = $elm$core$String$trim(model.tokenInput);
...
var $author$project$Chart$Heatmap$view = function (cfg) {
	...
	var cellSvg = function (c) {
		var tip = $author$project$Energy$dayLabel(c.day) + ('  ' + (pad2(c.hour) + (':00  ·  ' + ($elm$core$String$fromFloat(
			$elm$core$Basics$round(c.value * 10) / 10) + (' ' + cfg.unit)))));
		var col = A2(
			$elm$core$Maybe$withDefault,
			0,
			A2($elm$core$Dict$get, c.day, dayCol));
		return A2(
			$elm_community$typed_svg$TypedSvg$rect,
			_List_fromArray(
				[
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$x(col * cellW),
					$elm_community$typed_svg$TypedSvg$Attributes$InPx$y(c.hour * cellH),
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($elm_community$typed_svg$TypedSvg$Types$Translate, $author$project$Chart$Heatmap$pad.left, $author$project$Chart$Heatmap$pad.top)
							]))
					]),
				_Utils_ap(
					A2($elm$core$List$map, cellSvg, cfg.cells),
					_Utils_ap(
						focusOutline,
		...
		function (band, pairs) {
			var op = function () {
				...
				if (_v1.$ === 'Nothing') {
					...
					var h = _v1.a;
					return _Utils_eq(h, band.name) ? 1.0 : 0.18;
				}
			}();
			var areaPts = A3(
				$elm$core$List$map2,
	...
	var groupLabel = function (item) {
		return A2(
			...
			_List_fromArray(
				[
					$elm_community$typed_svg$TypedSvg$Core$text(
					item.node.name + (' · ' + (round1(
						A2($author$project$Chart$Treemap$groupPct, total, item.node.value)) + ' %')))
				]));
	};
	var leafSvg = function (item) {
		...
		var op = function () {
			...
			if (_v1.$ === 'Nothing') {
				...
				var h = _v1.a;
				return _Utils_eq(h, node.name) ? 1.0 : 0.25;
			}
		}();
...
var $author$project$Main$chartsView = F2(
	function (model, sortedRows) {
		var treemapRows = function () {
			var _v1 = model.focusedDay;
			if (_v1.$ === 'Just') {
				var d = _v1.a;
				return A2(
					$elm$core$List$filter,
		...
		var heatCells = A2($author$project$Energy$binHourly, model.metric, sortedRows);
